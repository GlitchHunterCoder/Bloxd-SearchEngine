(() => {
  ...
  style.textContent = `
    ...
    * {
    ...
    .card.selected {
      ...
      background: linear-gradient(180deg, #222840, var(--panel-2));
    }
  ...
  main.innerHTML = `
    <div class="panel" id="joinPanel" style="display:none;">
      <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <strong id="joinTitle">Selected game</strong>
    
        <input
          id="lobbyInput"
          placeholder="Lobby name or number (optional)"
          style="
            flex:1;
            min-width:180px;
            background: var(--panel-2);
  ...
  joinBtn.addEventListener('click', e => {
    ...
    const schemId = selectedGame.schematicId;
    const lobby   = lobbyInput.value.trim();
    let url = `https://bloxd.io/${lobby?"play":"game"}/classic_playerSchematic%7C${schemId}`;
    if (lobby) url += `/${encodeURIComponent(lobby)}`;
  ...
  function renderResults(arr) {
    ...
    for (const item of arr) {
      ...
      card.addEventListener('click', () => {
        ...
        joinTitle.textContent = `Join: ${item.name}`;
  ...
  function matchField(node, x, Search) {
    ...
    switch (node.operator) {
      ...
      default: {
        const t = node.rawValue.toLowerCase();
        if (!t) return false;
        ...
        const score =
          (valStr === t ? 10 :
           valStr.startsWith(t) ? 5 :
           2);
      
        Search._fuzzyScore = (Search._fuzzyScore || 0) + score;
        return true;
      }
    }
  }
